"""
C-Dataset 4 变体对比: 1D vs 128D GiGs × RandomForest vs XGBoost
 输入: data/C-Dataset/Evaluation/train.csv (18 维 MiRAGE 特征)
       code/model/gigs_dataC.pkl (GiGs 嵌入)
 输出: results/contribution_demo_C/  (汇总 + 对比图 + paper-ready 文案)
"""
import os
import pickle
import numpy as np
...
RESULTS_DIR = "results/contribution_demo_C"
os.makedirs(RESULTS_DIR, exist_ok=True)

# C-Dataset: 1 疾病相似度 + 7 药物相似度 = 18 维
FEATURE_18 = [
...
def _to_lookup_key(v, mapping):
    """兼容整数/字符串 ID (C-Dataset 是整数, F-Dataset 是字符串)"""
...
def main():
    print("=" * 70)
    print(" C-Dataset: 4 变体对比 (1D/128D × RF/XGBoost)")
    print("=" * 70)
    ...
    print(f"\n{'='*90}")
    print(f" C-Dataset 4 变体对比 (5-Fold CV, Mean ± Std)")
    print(f"{'='*90}")
