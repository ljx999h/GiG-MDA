...
TEST_CSV  = "data/F-Dataset/Evaluation/test.csv"
GIGS_PKL  = "code/model/gigs_dataF.pkl"
RESULTS_DIR = "results/F_Dataset"
...
def inject_gigs(df, gigs_data):
    ...
    # F-Dataset 用字符串 ID (DrugBank 字符串 / OMIM 字符串)
    # 自动兼容整数/字符串: 先尝试原值, 再 str, 最后 int
    def _to_lookup_key(v, mapping):
        # 1. 原值
        if v in mapping:
            return mapping[v]
        # 2. str 化 (F-Dataset 的 DrugBank/OMIM 字符串走这条)
        v_str = str(v).strip()
        if v_str in mapping:
            return mapping[v_str]
        # 3. int 化 (整数 ID 数据集, 如 DDCD 兼容)
        try:
            v_int = int(v)
            if v_int in mapping:
...
def main():
    print("=" * 70)
    print(" F-Dataset 一站式评估 (RandomForest + 5-Fold CV (论文方法))")
    print("=" * 70)
    ...
    try:
        ...
        summary_4v, all_metrics, lifts = run_4_variants(X_dot, X_emb, y, dataset_name="F-Dataset")
...
def plot_results(summary_df):
    ...
    ax.set_title('F-Dataset Ablation Study (XGBoost GPU, 18D MiRAGE + 128D GiGs)',
